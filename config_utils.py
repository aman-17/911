...
def print_configuration(config: Dict[str, Any], variant_name: str) -> None:
    ...
    model_keys = ["type", "model_arch", "emb_dim", "n_heads", "n_layers", "hidden_dim", "n_kv_heads"]
    ...
    training_keys = ["batch_size", "num_epochs", "initial_lr", "weight_decay", "drop_rate"]
    ...
    data_keys = ["train_data", "vocab_size", "max_seq_length", "seq_length", "stride", 
                 "num_workers", "pin_memory", "shuffle_buffer_size"]
    ...
    attention_keys = ["attention", "use_flash_attn", "qkv_bias", "rope", "tie_embeddings", 
                     "dtype", "softcap", "query_pre_attn_scalar"]
    for key in attention_keys:
        if key in config:
            print_config_value(key, config[key], 8)
    print("    ),")

    rope_keys = ["rope_theta", "rope_factor", "qk_rope_head_dim", "qk_nope_head_dim", 
                 "v_head_dim", "beta_fast", "beta_slow"]
    ...
    mla_keys = ["compression_block_size", "compression_stride", "selection_block_size", 
                "selection_top_k", "window_size", "q_lora_rank", "kv_lora_rank"]
    ...
    if all(key in config for key in ["emb_dim", "n_layers", "vocab_size"]):
        ...
        print(f"{Colors.CYAN}Total parameters:{Colors.RESET}         {Colors.MAGENTA}{total_params:,}{Colors.RESET} ({Colors.MAGENTA}{total_params:,}{Colors.RESET} active)")
        print(f"{Colors.CYAN}Non-embedding parameters:{Colors.RESET} {Colors.MAGENTA}{non_emb_params:,}{Colors.RESET} ({Colors.MAGENTA}{non_emb_params:,}{Colors.RESET} active)")
