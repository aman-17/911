...
class Scheduler(metaclass=ABCMeta):
    ...
    def set_lr(
        self, group: Dict[str, Any], trainer: "Trainer"
    ) -> Union[float, torch.Tensor]:
        """
...
class WSD(Scheduler):
    ...
    def __post_init__(self):
        ...
        if self.decay_fraction is not None and (
            ...
            raise ConfigurationError("decay_fraction must be between 0 and 1.")
...
class CosWithWarmup(Scheduler):
    ...
    def get_lr(
        ...
        if current < warmup:
            ...
            t_max = t_max - warmup
            return (
                eta_min + (initial_lr - eta_min) * (1 + cos(pi * current / t_max)) / 2
            )
    ...
    def get_lr(
        ...
        if current >= t_max - decay:
            ...
            return _linear_decay(
                final_cosine_lr, t_max - current, decay, self.decay_min_lr
            )
...
class SequentialScheduler(Scheduler):
    ...
    """
    schedulers_max_steps: Optional[List[int]] = (
        None  # deprecated, use 'schedulers_max' instead.
    )
    ...
    def __post_init__(self):
        ...
        if self.schedulers_max is None:
            raise ConfigurationError("'schedulers_max' must be specified")
    ...
    def get_lr(
        ...
        for scheduler, scheduler_max in zip(
            self.schedulers[:-1], self.schedulers_max, strict=True
        ):
