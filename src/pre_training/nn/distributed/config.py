...
from pre_training.callbacks.exceptions import ConfigurationError
    ...
    def merge(self, dotlist: List[str], prefix: Optional[str] = None, strict: bool = True) -> Self:
        ...
        try:
            ...
            if prefix is not None:
                dotlist = [o.replace(f"{prefix}.", "", 1) for o in dotlist if o.startswith(f"{prefix}.")]
            if not strict:
                field_names = set(f.name for f in fields(self))
                dotlist = [o for o in dotlist if any([o.startswith(f"{name}=") or o.startswith(f"{name}.") for name in field_names])]
    ...
    def from_dict(cls: Type[C], data: Dict[str, Any], overrides: Optional[List[str]] = None) -> C:
        ...
        def clean_data(d: Any, prefix: str) -> Any:
            if isinstance(d, dict):
                new_dict = {k: clean_data(v, f"{prefix}.{k}" if prefix else k) for k, v in d.items() if k != cls.CLASS_NAME_FIELD}
                if (cls_name := d.get(cls.CLASS_NAME_FIELD)) is not None and (cls_o := resolve_cls(cls_name)) is not None:
                ...
                return d.__class__((clean_data(x, f"{prefix}.{i}" if prefix else str(i)) for i, x in enumerate(d)))
            else:
                return d
