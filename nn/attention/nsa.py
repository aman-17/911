...
class NativeSparseAttention(nn.Module):
    ...
    def __init__(
        ...
        self.w_pe_compress = nn.Parameter(torch.randn(compression_block_size, self.n_kv_heads * self.head_dim))
    ...
    def _compress_tokens(
        ...
        if seq_len <= self.compression_block_size: 
            return keys, values # if sequence is too short, just return the original keys/values
        ...
        for i in range(seq_len):
            ...
            for j in range(self.selection_top_k):
                if j < top_indices.shape[-1]:
                    ...
                    end_idx = torch.min(start_idx + self.selection_block_size, torch.tensor(seq_len))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        selected_keys, selected_values = self._select_tokens(
            queries, keys, values, compressed_keys, compressed_values, batch_size, seq_len
        )
        ...
        for i in range(seq_len):
            if i < len(selected_keys):
                k_sel = selected_keys[i].view(batch_size, -1, self.n_kv_heads, self.head_dim).transpose(1, 2)
                v_sel = selected_values[i].view(batch_size, -1, self.n_kv_heads, self.head_dim).transpose(1, 2)
                
                if self.n_kv_heads < self.num_heads:
                    k_sel = k_sel.repeat_interleave(self.num_kv_groups, dim=1)
                    v_sel = v_sel.repeat_interleave(self.num_kv_groups, dim=1)
                
                scores_sel = q_sel[:, :, i:i+1, :] @ k_sel.transpose(2, 3) / math.sqrt(self.head_dim)
                attn_sel = F.softmax(scores_sel, dim=-1)
                attn_sel = self.dropout(attn_sel)
                
                out_i = (attn_sel @ v_sel).squeeze(2).transpose(1, 2).contiguous().view(batch_size, self.d_out)
                out_select[:, i, :] = out_i
